...
if _available_models_override:
    ...
    if invalid_models and not is_sglang_backend():
        raise RuntimeError(
            f"❌ AVAILABLE_MODELS_OVERRIDE inválido: {invalid_models}. "
            f"Modelos válidos: {sorted(known_models)}"
        )
    if invalid_models and is_sglang_backend():
        print(f"🔧 LLM_API_FORMAT=sglang: aceitando model-paths fora de MODEL_POOLS: {invalid_models}")
