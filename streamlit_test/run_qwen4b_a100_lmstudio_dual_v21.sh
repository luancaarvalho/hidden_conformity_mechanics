...
OUT_ROOT="${OUT_ROOT:-$HOME/luan/projeto_final/streamlit_test/batch_outputs/A100/qwen4b_dual_prompt_v21_seed1_20}"
...
STD_IDENTIFIER="${STD_IDENTIFIER:-qwen4b_std_v21}"
CG_IDENTIFIER="${CG_IDENTIFIER:-qwen4b_cg_v21}"
...
lms load --exact -y --identifier "$STD_IDENTIFIER" qwen/qwen3-4b
lms load --exact -y --identifier "$CG_IDENTIFIER" qwen/qwen3-4b
lms ps

python "$ROOT_DIR/run_qwen4b_a100_worker.py" \
  --base-url "$BASE_URL" \
  --model "$STD_IDENTIFIER" \
